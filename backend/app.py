...
# =============================================
# 🔧 CORRIGIR CAMPOS NO backend/app.py ou main.py
# Substituir as funções list_agents e list_workflows
# =============================================
...
# =============================================
# 🧪 ROTA DE TESTE: Testar agente
# =============================================

@app.post("/api/agents/{agent_id}/test", tags=["Chat"])
async def test_agent(agent_id: int, user_id: int = Query(1)):
    """🧪 Testar se agente está funcionando"""
    logger.info(f"🧪 Testando agente {agent_id}")

    try:
        async with async_session() as session:
            # Buscar agente
            query = sa.text("""
                SELECT id, name, model_provider, model_id, is_active
                FROM agno_agents 
                WHERE id = :agent_id AND user_id = :user_id
            """)

            result = await session.execute(query, {"agent_id": agent_id, "user_id": user_id})
            agent = result.fetchone()
            ...
            test_result = {
                ...
                "agno_service_available": USING_REAL_AGNO,
                "status": "ready" if agent.is_active else "inactive",
                "timestamp": datetime.now().isoformat()
            }

            logger.info(f"✅ Agente {agent_id} testado com sucesso")
            return test_result
